--[[
	2t1 Studio - ESP Module  (v2)

	Drawing API based. One pooled set of drawings per player, one render loop.

	Performance notes
	  - every Drawing write is dirty-checked, so unchanged properties never
	    cross into the executor
	  - visibility raycasts run on their own throttle, not once per frame
	  - Highlight properties are only written when they actually change
	  - hidden pools short-circuit instead of re-hiding sixty objects a frame
...
ESP.Settings = {
	...
	Box = {
		Enabled = true, Style = "corner", Thickness = 1,
		Outline = true, Filled = false, FillAlpha = 0.82,
	},
	Name = {
		Enabled = true, Size = 13, Font = 2,
		Outline = true, UseDisplay = true, ShowHealth = false,
	},
	Distance = { Enabled = true, Size = 12, Font = 2, Outline = true },
	Health = {
		Enabled = true, Side = "left", Width = 3, Offset = 5, Outline = true,
		HighColor = Color3.fromRGB(80, 230, 120),
		LowColor  = Color3.fromRGB(240, 70, 70),
	},
	Tracer   = { Enabled = false, Origin = "bottom", Thickness = 1, Outline = true },
	Skeleton = { Enabled = false, Thickness = 1, Color = Color3.fromRGB(235, 238, 246) },
	HeadDot  = { Enabled = false, Radius = 4, Thickness = 1, Filled = false, Sides = 12 },
	Chams = {
		Enabled = false,
		FillColor = Color3.fromRGB(255, 70, 80), FillTransparency = 0.6,
		OutlineColor = Color3.fromRGB(255, 255, 255), OutlineAlpha = 0,
		AlwaysOnTop = true,
	},
	OffScreen = { Enabled = false, Radius = 180, Size = 13, Thickness = 2 },
}
...
-- Some executors want numbers where the spec says boolean.
...
local function probe(kind)
	...
	pcall(function() obj:Remove() end)
	return numeric and "num" or false
end
...
if hasDrawing then
	for kind in pairs(ESP.Supported) do
		...
		elseif mode == "num" then
			warn("[2t1 ESP] " .. kind .. " needs numeric booleans here, using a compatibility wrapper.")
		end
	end
	if #ESP.Missing > 0 then
		warn("[2t1 ESP] Unsupported Drawing types: " .. table.concat(ESP.Missing, ", "))
	end
	if not ESP.Supported.Square then
		S.Box.Style = "corner"
		S.Box.Filled = false
	end
	if not ESP.Supported.Circle then S.HeadDot.Enabled = false end
	if not ESP.Supported.Triangle then S.OffScreen.Enabled = false end
end
...
function ESP:Diagnose()
	...
	local okHl = pcall(function() local h = Instance.new("Highlight"); h:Destroy() end)
...
local function rawNew(kind)
	...
	if ESP.Mode[kind] == "num" then return boolProxy(obj), true end
	return obj, true
end

-- create and configure a drawing; a failure retires that type quietly
local function build(kind, setup)
	local obj, real = rawNew(kind)
	if not real then return Draw.new(obj) end

	local ok = pcall(setup, obj)
	if ok then return Draw.new(obj) end
	...
	for _, k in ipairs(ESP.Missing) do
		if k == kind then dup = true break end
	end
	if not dup then table.insert(ESP.Missing, kind) end
	warn("[2t1 ESP] " .. kind .. " could not be configured, disabling it.")
	return Draw.new(makeDummy())
end

local function newLine()
	return build("Line", function(l)
		l.Visible = false; l.Thickness = 1; l.Transparency = 1; l.Color = WHITE
	end)
end
...
local function newText()
	return build("Text", function(t)
		...
		t.Size = 13; t.Font = 2; t.Transparency = 1; t.Color = WHITE
	end)
end
...
local function newSquare()
	return build("Square", function(sq)
		...
		sq.Transparency = 1; sq.Color = WHITE
	end)
end
...
local function newCircle()
	return build("Circle", function(c)
		...
		c.NumSides = 12; c.Radius = 4; c.Transparency = 1; c.Color = WHITE
	end)
end
...
local function newTriangle()
	return build("Triangle", function(t)
		...
		t.Transparency = 1; t.Color = WHITE
	end)
end
...
local function ensureHighlight(p, char)
	...
	if st.FillColor ~= c.FillColor then
		st.FillColor = c.FillColor
		hl.FillColor = c.FillColor
	end
	if st.FillTransparency ~= c.FillTransparency then
		st.FillTransparency = c.FillTransparency
		hl.FillTransparency = c.FillTransparency
	end
	if st.OutlineColor ~= c.OutlineColor then
		st.OutlineColor = c.OutlineColor
		hl.OutlineColor = c.OutlineColor
	end
	if st.OutlineAlpha ~= c.OutlineAlpha then
		st.OutlineAlpha = c.OutlineAlpha
		hl.OutlineTransparency = c.OutlineAlpha
	end

	local depth = c.AlwaysOnTop
		and Enum.HighlightDepthMode.AlwaysOnTop
		or Enum.HighlightDepthMode.Occluded
	if st.Depth ~= depth then
		st.Depth = depth
		hl.DepthMode = depth
	end
end
...
-- DRAW HELPERS
-- ============================================
local function drawCornerBox(p, x, y, w, h, color, thick)
	local cut = (w < h and w or h) * 0.28
	local outline = S.Box.Outline
	local outThick = thick + 2

	local rx, by = x + w, y + h

	local fromPts = {
		Vector2.new(x, y),   Vector2.new(x, y),
		Vector2.new(rx, y),  Vector2.new(rx, y),
		Vector2.new(x, by),  Vector2.new(x, by),
		Vector2.new(rx, by), Vector2.new(rx, by),
	}
	local toPts = {
		Vector2.new(x + cut, y),   Vector2.new(x, y + cut),
		Vector2.new(rx - cut, y),  Vector2.new(rx, y + cut),
		Vector2.new(x + cut, by),  Vector2.new(x, by - cut),
		Vector2.new(rx - cut, by), Vector2.new(rx, by - cut),
	}

	for i = 1, 8 do
		local o = p.cornerOut[i]
		if outline then
			o:set("From", fromPts[i])
			o:set("To", toPts[i])
			o:set("Thickness", outThick)
			o:set("Color", BLACK)
		...
		local l = p.corners[i]
		l:set("From", fromPts[i])
		l:set("To", toPts[i])
		l:set("Thickness", thick)
		l:set("Color", color)
		l:show(true)
	end
end
...
local function drawArrow(p, worldPos, color, viewW, viewH)
	...
	p.arrow:set("PointB", Vector2.new(
		tipX - dx * sz * 0.4 + px * sz * 0.62,
		tipY - dy * sz * 0.4 + py * sz * 0.62))
	p.arrow:set("PointC", Vector2.new(
		tipX - dx * sz * 0.4 - px * sz * 0.62,
		tipY - dy * sz * 0.4 - py * sz * 0.62))
...
local function renderPlayer(plr, p, ctx)
	...
	-- part lookups stay cached until the model is replaced
	...
	if S.VisibilityCheck then
		...
		color = sameTeam and S.Colors.Team or S.Colors.Enemy
	end
	...
	local topWorld = (headCF + headCF.UpVector * (head.Size.Y * 0.9)).Position
	...
	-- ----- tracer -----
	if S.Tracer.Enabled and ctx.tracerFrom then
		local to = Vector2.new(bx + width * 0.5, by + height)
		if S.Tracer.Outline then
			p.tracerOut:set("From", ctx.tracerFrom)
			p.tracerOut:set("To", to)
			p.tracerOut:set("Thickness", S.Tracer.Thickness + 2)
			p.tracerOut:set("Color", BLACK)
	...
	if S.Skeleton.Enabled then
		...
		local skelColor = S.Skeleton.Color
		local skelThick = S.Skeleton.Thickness
		local n = #bones
		...
		for i = 1, n do
			...
			if a and b then
				...
				if pa.Z > 0 and pb.Z > 0 then
					...
					line:set("Thickness", skelThick)
					line:set("Color", skelColor)
					line:show(true)
				else
					line:show(false)
				end
...
function ESP:Init()
	...
	if not hasDrawing then
		warn("[2t1 ESP] Drawing API not available. ESP disabled.")
		return false, "no drawing api"
	end
	if not ESP.Supported.Line or not ESP.Supported.Text then
		warn("[2t1 ESP] Core Drawing types missing. ESP disabled.")
	...
	conn = RunService.RenderStepped:Connect(function(dt)
		...
		-- resolve shared state once per frame
		...
		if limit and limit > 0 and ctx.myPos then
			-- refresh the nearest-N set a few times a second, not every frame
			...
			if sortAccum >= 0.2 then
				...
				renderOrder = {}
				local cap = limit < #list and limit or #list
				for i = 1, cap do
...
function ESP:Stop()
	...
	for _, p in pairs(objects) do pcall(hidePool, p) end
end
...
function ESP:Refresh()
	for plr, p in pairs(objects) do
		pcall(destroyPool, p)
		local ok, pool = pcall(createPool)
		objects[plr] = ok and pool or nil
	end
end
...
function ESP:BuildUI(tab)
	...
	if #ESP.Missing > 0 then
		main:Paragraph({
			...
			Content = "Your executor does not provide these Drawing types: "
				...
				.. ". Features that rely on them are hidden. Everything else works normally."
		})
	elseif #wrapped > 0 then
		main:Paragraph({
			Title = "Compatibility mode",
			Content = "These Drawing types needed a small wrapper on your executor: "
				.. table.concat(wrapped, ", ")
				.. ". Everything still works, there is nothing for you to change."
		})
	end

	main:Toggle({
		Name = "Enabled", Icon = "eye", Flag = "esp_enabled", Default = false,
		Tooltip = "Master switch. Nothing below draws while this is off.",
	...
	main:Slider({
		...
		Tooltip = "Anything past this range is skipped before any work is done.",
	...
	perf:Dropdown({
		...
		Default = "Every frame",
		Tooltip = "How often the overlay is recalculated. 30 Hz is hard to tell apart and roughly halves the cost.",
		Callback = function(v)
			S.RefreshRate = (v == "60 Hz" and 1/60)
				or (v == "30 Hz" and 1/30)
				or (v == "20 Hz" and 1/20)
				or (v == "15 Hz" and 1/15)
				or (v == "10 Hz" and 1/10)
	...
	col:ColorPicker({
		...
		Tooltip = "Used when you have line of sight to the target.",
	...
	box:Toggle({
		Name = "Enabled", Icon = "box", Flag = "esp_box", Default = true,
	...
	box:Dropdown({
		...
		Default = "corner",
		Tooltip = "Corner draws eight bracket segments. Full draws a complete rectangle.",
		Callback = function(v) S.Box.Style = v end
	})
	box:Slider({
		Name = "Thickness", Icon = "sliders", Flag = "esp_box_thick",
		Min = 1, Max = 5, Default = 1,
		Callback = function(v) S.Box.Thickness = v end
	})
	box:Toggle({
		Name = "Outline", Icon = "layers", Flag = "esp_box_outline", Default = true,
		Tooltip = "Dark border so the box stays readable on bright maps. Doubles the line count.",
		Callback = function(v) S.Box.Outline = v end
	})

	if ESP.Supported.Square then
		box:Toggle({
			Name = "Filled", Icon = "palette", Flag = "esp_box_filled", Default = false,
			Callback = function(v) S.Box.Filled = v end
		})
		box:Slider({
			Name = "Fill Opacity", Icon = "sliders", Flag = "esp_box_alpha",
	...
	txt:Toggle({
		Name = "Name", Icon = "user", Flag = "esp_name", Default = true,
		Callback = function(v) S.Name.Enabled = v end
	})
	txt:Toggle({
		Name = "Use Display Name", Icon = "user", Flag = "esp_name_display", Default = true,
		Tooltip = "Off shows the @username instead.",
		Callback = function(v) S.Name.UseDisplay = v end
	})
	txt:Toggle({
		Name = "Append Health", Icon = "heart", Flag = "esp_name_hp", Default = false,
		Tooltip = "Adds current health next to the name.",
		Callback = function(v) S.Name.ShowHealth = v end
	})
	txt:Slider({
		Name = "Name Size", Icon = "type", Flag = "esp_name_size",
	...
	txt:Toggle({
		Name = "Distance", Icon = "move", Flag = "esp_dist", Default = true,
		Callback = function(v) S.Distance.Enabled = v end
	})
	txt:Slider({
		Name = "Distance Size", Icon = "type", Flag = "esp_dist_size",
	...
	tr:Toggle({
		Name = "Enabled", Icon = "radar", Flag = "esp_tracer", Default = false,
		Callback = function(v) S.Tracer.Enabled = v end
	})
	tr:Dropdown({
		Name = "Origin", Icon = "compass", Flag = "esp_tracer_origin",
		List = { "bottom", "center", "top", "mouse" }, Default = "bottom",
		Tooltip = "Where each line starts on your screen.",
		Callback = function(v) S.Tracer.Origin = v end
	})
	tr:Slider({
		Name = "Thickness", Icon = "sliders", Flag = "esp_tracer_thick",
		Min = 1, Max = 5, Default = 1,
		Callback = function(v) S.Tracer.Thickness = v end
	})
	tr:Toggle({
		Name = "Outline", Icon = "layers", Flag = "esp_tracer_outline", Default = true,
	...
	ex:Toggle({
		Name = "Skeleton", Icon = "activity", Flag = "esp_skeleton", Default = false,
	...
	ex:ColorPicker({
		Name = "Skeleton Colour", Icon = "palette", Flag = "esp_skel_col",
	...
	if ESP.Supported.Circle then
		...
		ex:Toggle({
			Name = "Dot Filled", Icon = "palette", Flag = "esp_headdot_fill", Default = false,
	...
	if ESP.Supported.Triangle then
		ex:Divider()
		ex:Toggle({
			Name = "Off-Screen Arrows", Icon = "compass", Flag = "esp_offscreen", Default = false,
			Tooltip = "Points toward targets outside your view.",
			Callback = function(v) S.OffScreen.Enabled = v end
		})
		ex:Slider({
			Name = "Arrow Distance", Icon = "move", Flag = "esp_offscreen_r",
			Min = 60, Max = 400, Default = 180,
			Callback = function(v) S.OffScreen.Radius = v end
		})
		ex:Slider({
			Name = "Arrow Size", Icon = "sliders", Flag = "esp_offscreen_s",
	...
	ch:Toggle({
		Name = "Enabled", Icon = "layers", Flag = "esp_chams", Default = false,
		Tooltip = "Fills the character model with colour, visible through walls. Rendered by the engine, so it costs GPU rather than script time.",
		Callback = function(v) S.Chams.Enabled = v end
	})
	ch:Toggle({
		Name = "Always On Top", Icon = "eye", Flag = "esp_chams_top", Default = true,
		Tooltip = "Off means the fill only shows where the model is already visible.",
		Callback = function(v) S.Chams.AlwaysOnTop = v end
	})
	ch:ColorPicker({
		Name = "Fill Colour", Icon = "palette", Flag = "esp_chams_fill",
		Default = S.Chams.FillColor,
		Callback = function(c) S.Chams.FillColor = c end
	})
	ch:Slider({
		Name = "Fill Opacity", Icon = "sliders", Flag = "esp_chams_alpha",
		Min = 0, Max = 1, Default = 0.4, Rounding = 2,
		Callback = function(v) S.Chams.FillTransparency = 1 - v end
	})
	ch:ColorPicker({
		Name = "Outline Colour", Icon = "palette", Flag = "esp_chams_out",
		Default = S.Chams.OutlineColor,
		Callback = function(c) S.Chams.OutlineColor = c end
	})
	ch:Slider({
		Name = "Outline Opacity", Icon = "sliders", Flag = "esp_chams_outa",
	...
	mt:Button({
		Name = "Capability Report", Icon = "terminal",
		Tooltip = "Prints which Drawing types your executor supports to the console.",
		Callback = function() ESP:Diagnose() end
	})
	mt:Button({
		Name = "Rebuild Drawings", Icon = "refresh",
		Tooltip = "Recreates every drawing object. Use if the overlay gets stuck.",
		Callback = function() ESP:Refresh() end
	})
	mt:Button({
		Name = "Clear All", Icon = "trash",
	...
	return {
		...
		Text = txt, Health = hp, Tracers = tr, Extras = ex,
		Chams = ch, Maintenance = mt
	}
end
